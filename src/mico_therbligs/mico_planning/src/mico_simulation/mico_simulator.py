...
class mico_simulator:
    ...
    def __init__(self, name):
        ...
        slef.clientID=vrep.simxStart('127.0.0.1',19999,True,True,5000,5) # Connect to V-REP
    ...
    def start_simulation(self):
        ...
        for i in range(6):
            ...
            res, han = vrep.simxGetObjectHandle(clientID,joint_name, vrep.simx_opmode_oneshot_wait)
            if res == -1:
    ...
    def set_arm_velocity(self, vel_vector):
        for ind, vel in enumerate(vel_vector):
            ret = vrep.simxSetJointTargetVelocity(clientID,self.handles[ind],vel,vrep.simx_opmode_oneshot_wait)
    ...
    def move_arm(self, target_position):
        for ind, pos in enumerate(target_position):
            ret = vrep.simxSetJointTargetPosition(clientID, self.handles[ind], pos, vrep.simx_opmode_oneshot_wait)
            if ret != vrep.simx_return_ok:
